...
import { OPPORTUNITY_TYPES, ALL_SKILLS } from '@/utils';
...
export default function IndustryOpportunitiesPage() {
  ...
  const openEdit = (opp: Opportunity) => {
    ...
    setForm({
      ...
      num_positions: opp.num_positions,
  ...
  const handleSave = async () => {
    ...
    toast.error(
      editing
        ? 'Opportunity update API is not connected yet.'
        : 'Opportunity creation API is not connected yet.'
    );
  };
  ...
  const handleDelete = async (_id: string) => {
    ...
    toast.error('Opportunity delete API is not connected yet.');
  };
  ...
  if (error) {
    return <ErrorState message={error} onRetry={fetchOpps} />;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {opportunities.length === 0 ? (
        <Card>
          <EmptyState
            icon={<Briefcase className="h-8 w-8 text-brand-primary/40" />}
              ...
              <div className="flex items-center justify-between px-1">
                ...
                <div className="flex gap-1">
                  <button
                    ...
                    className="rounded-lg p-2 text-ink-muted hover:bg-brand-primary/5 hover:text-brand-primary"
                  >
                  ...
                  <button
                    ...
                    className="rounded-lg p-2 text-ink-muted hover:bg-red-50 hover:text-error"
                  >
      ...
      <Modal
        ...
        title={editing ? 'Edit Opportunity' : 'Post New Opportunity'}
        size="xl"
      >
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <Input
              ...
              onChange={(e) =>
                setForm({
                  ...form,
                  num_positions: parseInt(e.target.value) || 1,
                })
              }
            />
          </div>
            ...
            <div className="flex flex-wrap gap-1.5 rounded-xl border border-surface-border p-3">
              {form.required_skills.map((s) => (
                <button
                  ...
                  onClick={() =>
                    setForm({
                      ...form,
                      required_skills: form.required_skills.filter(
                        (x) => x !== s
                      ),
                    })
                  }
              ...
              <Select
                ...
                onChange={(e) => {
                  if (
                    e.target.value &&
                    !form.required_skills.includes(e.target.value)
                  ) {
                    setForm({
                      ...form,
                      required_skills: [
                ...
                className="min-w-[120px] flex-1 border-0 p-0 text-sm"
              >
            ...
            <div className="flex flex-wrap gap-1.5 rounded-xl border border-surface-border p-3">
              {form.preferred_skills.map((s) => (
                <button
                  ...
                  onClick={() =>
                    setForm({
                      ...form,
                      preferred_skills: form.preferred_skills.filter(
                        (x) => x !== s
                      ),
                    })
                  }
              ...
              <Select
                ...
                onChange={(e) => {
                  if (
                    e.target.value &&
                    !form.preferred_skills.includes(e.target.value)
                  ) {
                    setForm({
                      ...form,
                      preferred_skills: [
              ...
              >
                ...
                {ALL_SKILLS.filter(
                  (s) => !form.preferred_skills.includes(s)
                ).map((s) => (
                  <option key={s} value={s}>
                    {s}
                  </option>
          ...
          <Button onClick={handleSave} className="w-full">
            {editing ? 'Update' : 'Post'} Opportunity
          </Button>
        </div>
      </Modal>
    </div>
