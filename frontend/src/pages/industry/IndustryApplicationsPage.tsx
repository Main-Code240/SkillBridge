...
export default function IndustryApplicationsPage() {
  ...
  const updateStatus = async (
    ...
    try {
      toast.error(
        'Application status API is not connected yet.'
      );
    } finally {
      setUpdating(false);
    }
            ...
            <button
              ...
              className={`rounded-lg px-3 py-1.5 text-sm font-medium transition-all ${
                filter === s
                  ...
                  : 'border border-surface-border bg-white text-ink-muted hover:bg-gray-50'
              }`}
            >
              {titleCase(s)}
            </button>
      ...
      {filtered.length === 0 ? (
        <Card>
          <EmptyState
            ...
            description="Applications will appear here once students apply to your opportunities."
          />
        </Card>
      ) : (
