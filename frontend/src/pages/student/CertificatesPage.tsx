    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-ink-muted">Manage your certifications and achievements</p>
        </div>
        <Button onClick={() => setShowAdd(true)}><Plus className="h-4 w-4" /> Add Certificate</Button>
      </div>

      {certificates.length === 0 ? (
        <Card><EmptyState icon={<Award className="h-8 w-8 text-brand-primary/40" />} title="No certificates yet"
          description="Add your certifications to showcase your achievements."
          action={<Button onClick={() => setShowAdd(true)}><Plus className="h-4 w-4" /> Add Your First Certificate</Button>} /></Card>
      ) : (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {certificates.map((c) => (
            <Card key={c.id} className="p-5">
              ...
              {c.issue_date && <p className="mt-1 text-xs text-ink-muted">Issued: {formatDate(c.issue_date)}</p>}
              {c.skill_name && <Badge variant="brand">{c.skill_name}</Badge>}
              {c.certificate_url && (
                <a href={c.certificate_url} target="_blank" rel="noreferrer" className="mt-3 flex items-center gap-1 text-sm text-brand-primary hover:underline">
      ...
      <Modal isOpen={showAdd} onClose={() => setShowAdd(false)} title="Add Certificate">
